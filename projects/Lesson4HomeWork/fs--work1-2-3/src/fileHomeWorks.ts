...
/**
 * Work-2 - Create a function that copies a file to another location.
 * @param file - path to file
 * @param path - path to copy
 */
function cpCommand(file: string, path: string) {
  if (!file || !path) {
    console.log('Please provide a path');
    return;
  }
  const parsedPath = file.split('/');
  fs.copyFile(file, `${file}/${parsedPath[parsedPath.length - 1]}`, (err) => {
    if (err) {
      console.log(err);
    } else {
...
/**
 * Work-Bonus - Create a function that copies a text from a file to another file.
...
export {addNewTextToFile, copyTextFromFile, zipFolder, cpCommand}
